...
resource "aws_iam_role" "gateway_execution" {
  name = "${var.stack_name}-gateway-execution-role"
...
# Inline policy for gateway execution
resource "aws_iam_role_policy" "gateway_execution" {
  name = "GatewayExecutionPolicy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      # CloudWatch Logs
      {
        ...
        Resource = "arn:aws:logs:${data.aws_region.current.id}:${data.aws_caller_identity.current.id}:log-group:/aws/bedrock-agentcore/gateways/*"
      },
      # X-Ray Tracing
      {
        Sid    = "XRayTracing"
