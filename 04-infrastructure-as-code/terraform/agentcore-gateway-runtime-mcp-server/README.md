...
- Exposing MCP tools through Gateway for agent-to-agent communication

## Gateway Use Case

The included AgentCore Gateway demonstrates how to expose MCP server tools for consumption by other agents:

- **Agent-to-Agent Communication**: Other agents can discover and invoke the three tools through the Gateway
- **Unified Authentication**: Gateway uses the same Cognito JWT authentication as the runtime
- **MCP Protocol Compatibility**: Gateway exposes tools using standard MCP protocol
- **Tool Discovery**: Agents can list available tools and their schemas through the Gateway
- **Scalable Architecture**: Multiple agents can consume the same tools without direct runtime access

This pattern is useful for:
- Building multi-agent systems where agents share common tools
- Creating reusable tool libraries accessible via MCP
- Implementing service-oriented agent architectures
- Centralizing tool management and access control
...
- **AgentCore Gateway**: Exposes the MCP server tools as MCP-compatible endpoints
  - Targets the AgentCore Runtime
  - Uses same JWT authentication as the runtime
  - Enables other agents to consume the tools via MCP protocol
- **IAM Roles**: 
...
- **AgentCore Gateway**: Exposes the MCP server tools as MCP-compatible endpoints for consumption by other agents
...
**Option A: Use Cognito (Default - No Extra Config)**
```hcl
auth_provider = "cognito"
# That's it! Cognito resources created automatically
```

**Option B: Use Entra ID**
```hcl
...
See [ENTRA_ID_SETUP.md](ENTRA_ID_SETUP.md) for Entra ID configuration details.
...
gateway_id = "GATEWAY1234567890"
gateway_arn = "arn:aws:bedrock-agentcore:us-west-2:123456789012:gateway/GATEWAY1234567890"
...
This pattern supports **flexible JWT-based authentication** with two options:

### Option 1: AWS Cognito (Default)

- **Automatic Setup**: Cognito User Pool created automatically by Terraform
- **Test User**: Pre-configured user (testuser/MyPassword123!)
- **Zero Configuration**: Works out of the box
- **Best For**: Quick testing, AWS-native deployments, simple authentication needs

### Option 2: Microsoft Entra ID

- **Enterprise Integration**: Use your existing Entra ID tenant
- **Production Ready**: Leverage enterprise identity management
- **Configuration Required**: Tenant ID and Client ID from Azure Portal
- **Best For**: Enterprise deployments, existing Entra ID infrastructure, multi-cloud scenarios

See [Choosing Your Authentication Provider](#choosing-your-authentication-provider) below for detailed guidance.

### Authentication Flow

1. User authenticates with identity provider (Cognito or Entra ID)
2. Identity provider issues JWT access token
3. Client includes JWT token in MCP request headers
4. Runtime/Gateway validates token using provider's OIDC discovery endpoint
5. Authorized requests processed by MCP server

### Other OIDC Providers

AgentCore supports any OIDC-compliant identity provider:
- **Okta** - See [03-integrations/IDP-examples/](../../../03-integrations/IDP-examples/)
- **Auth0**, Google Identity Platform, or any OIDC provider
- Configure by setting `auth_provider = "entra_id"` and providing the discovery URL

**Note**: This is a backend authentication pattern for MCP tool access. For user-facing applications, integrate with your identity provider's hosted UI for end-user authentication.
...
- You want to get started immediately without external setup
- Testing MCP server functionality is the priority
- You're building an AWS-native application

✅ **Simple Authentication Requirements**
- Basic username/password authentication is sufficient
- You don't have existing identity infrastructure
- You want AWS-managed authentication

✅ **AWS-Only Deployments**
- Your entire stack is on AWS
- You prefer AWS-native services
- You want simplified billing and management

### Use Entra ID When:

✅ **Enterprise Deployments**
- You have an existing Entra ID tenant
- You need enterprise identity features (SSO, MFA, Conditional Access)
- You want centralized identity management

✅ **Multi-Cloud Scenarios**
- Your organization uses Microsoft 365 or Azure
- You need consistent identity across AWS and Azure
- You want to leverage existing Entra ID investments

✅ **Compliance & Governance**
- You require enterprise-grade audit logging
- You need advanced security features (risk-based authentication)
- You must comply with specific identity standards

### Configuration Comparison

| Feature | Cognito | Entra ID |
|---------|---------|----------|
| Setup Time | Instant (automated) | 5-10 minutes (manual) |
| Configuration | None required | Tenant ID + Client ID |
| Test User | Auto-created | Use existing users |
| Cost | AWS Cognito pricing | Included with Microsoft 365 |
| MFA Support | Yes | Yes (more options) |
| SSO Integration | Limited | Extensive |
| Enterprise Features | Basic | Advanced |

### Switching Between Providers

You can easily switch authentication providers by changing one variable:
...
No other code changes required - the infrastructure adapts automatically!
...
TENANT_ID=$(terraform output -raw entra_id_tenant_id)
CLIENT_ID=$(terraform output -raw entra_id_client_id)

# Get authentication token (requires msal: pip install msal)
python get_entra_id_token.py $TENANT_ID $CLIENT_ID your-email@company.com YourPassword
```

This will output a JWT token. Copy the token for the next step.

### Step 2: Test the MCP Server (Direct)

Test the MCP server runtime directly:
...
Test the Gateway that exposes the MCP server tools:
...
# Test the Gateway (replace YOUR_JWT_TOKEN with the token from step 1)
python test_gateway.py $GATEWAY_ARN YOUR_JWT_TOKEN $REGION
```

The Gateway provides an MCP-compatible interface that other agents can consume to access the three tools (add_numbers, multiply_numbers, greet_user).
...
├── gateway.tf                   # AgentCore Gateway targeting the runtime
├── variables.tf                 # Input variables
├── outputs.tf                   # Output values (includes Cognito & Gateway)
...
├── get_entra_id_token.py        # Entra ID JWT token retrieval
├── test_mcp_server.py           # MCP server testing script (direct)
├── test_gateway.py              # Gateway testing script
├── ENTRA_ID_SETUP.md            # Entra ID setup guide
...
If Gateway tool invocations fail:

1. Verify the Gateway is in ACTIVE state:
   ```bash
   GATEWAY_ARN=$(terraform output -raw gateway_arn)
   aws bedrock-agentcore describe-gateway --gateway-arn $GATEWAY_ARN
   ```

2. Ensure the target runtime is healthy and accessible
3. Verify JWT authentication is configured correctly
4. Check that the Gateway and Runtime use the same Cognito configuration
5. Review CloudWatch Logs for both Gateway and Runtime
...
- Create multiple Gateways for different tool subsets
- Build multi-agent systems that consume Gateway tools
- Implement tool versioning and A/B testing through multiple Gateways
